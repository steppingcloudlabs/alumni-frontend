<template>
  <v-layout row wrap class="pa-5" v-if="render">
    <v-flex xs12>
      <v-card v-if="escalationManager && escalationManager.level" min-height="250px" light>
        <div
        ...
        <v-card-actions>
          ...
          <v-btn fab small text @click="showEmailDialog()">
            <v-icon color="blue">edit</v-icon>
          </v-btn>
        </v-card-actions>
      </v-card>
      <v-card v-else light min-height="250px" class="pa-0 ma-0">
        <v-card-title class="title pt-4 pl-3">Escalation Level 1</v-card-title>
        <div class="text-center">
          <v-card-text style="color: black; height: 150px; line-height: 120px">No manager added</v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
