  ...
  data() {
    return {
      items: [
        {
          text:
            "Upload the files in SFTP Server or Upload Zip Folder in the dialog",
        },
        {
          text:
            "File name should be UserId_fileType_. For example:1234_FIRSTMONTHSALARY",
        },
        {
          text:
            "There are 5 types of file that can be uploaded(Name should be in CAPS)",
        },
        {
          text:
            "FIRSTMONTHSALARY,SECONDMONTHSALARY,THIRDMONTHSALARY,EXPERIENCELETTER,RELIEVINGLETTER",
        },
      ],
      checkbox: false,
      uploadId: "",
      ...
      rules: [
        (value) =>
          ...
          "Zip File size should be less than 5 gB!",
      ],
    };
  },
  computed: {
  ...
  methods: {
    ...
    async uploadMultiPart() {
      ...
      for (let index = 1; index < CHUNK_COUNT + 1; index++) {
        ...
        // console.log(chunk)
        // let request = await this.$store.dispatch("adminModule/signedUrl", {
      ...
      this.$store
        .dispatch("adminModule/completeUpload", dat)
        .then((response) => {
          if ((response.status = "200")) {
            this.$store.commit("showSnackbar", {
